...
void random_update() {
    int index = rand() % (64 * 4096 * 16);
    arr[index] = (arr[index] + 1) * 101 % 256;
}
...
void tx(size_t p, void (*write_one)(), void (*write_zero)()) {
    ...
    while(1) {
        ...
        while (rdtsc_nofence() < t) random_update();
        t += p;
    }
}
...
int main(int argc, char** argv) {
    if (argc != 5) {
        printf("Wrong number of arguments: ./randomupdates method mode freq threshold\n");
        printf("Method can be: 'fr', 'ff', 'pp'\n");
        printf("Mode can be: 'tx', 'rx', 'test', 'benign'\n");
    ...
    } else if (strcmp(argv[2],"benign") == 0) {
        benign(period);
    } else {
        printf("Bad mode: must be 'tx', 'rx', 'test', or 'benign'\n");
        exit(0);
    }
}